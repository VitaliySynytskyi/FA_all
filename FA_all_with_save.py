...
layout1 = html.Div([
    ...
    dcc.Download(id="download-excel"), # <-- Added Download component for single save
    dcc.Download(id="download-batch-excel"), # <-- Added Download component for batch save
    dbc.Toast(
        id="click-toast",
        header="Attention",
        icon="danger",
...
@app.callback(
    Output("temp_seve_batch", "children"),  # Output 1: Status message
    Output("download-batch-excel", "data"), # Output 2: Data for download
...
def save_batch_results(n_clicks, n_size, split, condition, definition, min_dist_option, overlap_mode, batch_window_mode):
    if n_clicks is None or n_clicks < 1:
        return dash.no_update, dash.no_update # Return no_update for both outputs
    if not batch_results:
        return html.Div(["No batch results to save"], style={'color': 'orange'}), dash.no_update
    
    triggered_id = dash.callback_context.triggered[0]['prop_id'].split('.')[0]
    if triggered_id != 'save_batch':
    ...
    try:
        ...
        output_filename = "batch_results_n={}_split={}_cond={}_def={}_mindist={}_overlap={}_winmode={}.xlsx".format(
            n_size, split, condition, definition, min_dist_option, overlap_mode, batch_window_mode)
        
        # Save to Excel buffer
        output_buffer = io.BytesIO()
        with pd.ExcelWriter(output_buffer, engine='xlsxwriter') as writer:
            df_batch.to_excel(writer, index=False, sheet_name='BatchResults')
        output_buffer.seek(0)
        
        # Prepare data for download component
        download_data = dcc.send_bytes(output_buffer.read(), filename=output_filename)
        
        return html.Div([f"Batch results file ready: {output_filename}"], style={'color': 'green'}), download_data
    except Exception as e:
        print(f"Error saving batch results: {str(e)}")
        return html.Div([f"Error saving batch results: {str(e)}"], style={'color': 'red'}), dash.no_update
...
@app.callback([Output("temp_seve", "children"),
              ...
              [Input("save", "n_clicks")], # Removed other inputs as they are not needed for triggering download based on button click
...
def save(n_clicks, active_cell, page_current, ids, filename, n_size, w_min, w_s, w_e, w_max, fmin, opt, definition, min_dist_option, overlap_mode):
    if n_clicks is None or n_clicks < 1:
        return dash.no_update, dash.no_update 
    
    if filename is None:
        return html.Div(["No file selected to save"], style={'color': 'red'}), dash.no_update
    ...
    try:
        ...
        if definition == "dynamic":
            if df is not None:
                ...
                df_to_save = pd.DataFrame(columns=['rank', 'ngram', 'F', 'R', 'a', 'gamma', 'goodness'])
        else: # Static mode
            if df is not None:
                df_copy = df.copy()
                ...
                if len(df_copy) > 0:
                    ...
                    stats_cols = ['R_avg', 'dR', 'Rw_avg', 'dRw', 'gamma_avg', 'dgamma', 'gammaw_avg', 'dgammaw']
                    stats_vals = [R_avg, dR, Rw_avg, dRw, gamma_avg, dgamma, gammaw_avg, dgammaw]
                    for col, val in zip(stats_cols, stats_vals):
        ...
        # --- Generate Filename for Main File --- 
        main_filename = "{0}_condition={7}_fmin={1}_n={2}_w=({3},{4},{5},{6})_def={8}_mindist={9}_overlap={10}.xlsx".format(
            os.path.splitext(file)[0], fmin, n_size, w_min, w_s, w_e, w_max, opt, definition, min_dist_option, overlap_mode)

        # --- Save Main DataFrame to Buffer --- 
        main_buffer = io.BytesIO()
        with pd.ExcelWriter(main_buffer, engine='xlsxwriter') as writer:
            df_to_save.to_excel(writer, index=False, sheet_name='Results')
        main_buffer.seek(0)
        
        # --- Prepare Details DataFrame (if applicable) --- 
        df_details = None
        details_filename = None
        if active_cell: 
            if definition == "dynamic" and new_ngram and hasattr(new_ngram, 'dfa'):
                details_filename = "{}_new_ngram_details.xlsx".format(os.path.splitext(file)[0])
                df_details = pd.DataFrame()
                df_details["w"] = list(new_ngram.dfa.keys())
                df_details['∆F'] = list(new_ngram.dfa.values())
                df_details['fit=a*w^b'] = new_ngram.temp_dfa
            elif definition == "static":
                try:
                    row_index = active_cell['row']
                    if page_current is not None and page_current > 0:
                        row_index += page_current * 50
                    if ids is not None and row_index < len(ids):
                        selected_index_in_view = ids[row_index]
                        if selected_index_in_view < len(df):
                            ngram = df.iloc[selected_index_in_view]['ngram']
                            if ngram != 'new_ngram' and model and ngram in model:
                                details_filename = "{}_{}_details.xlsx".format(os.path.splitext(file)[0], ngram)
                                df_details = pd.DataFrame()
                                df_details["w"] = list(model[ngram].fa.keys())
                                df_details['∆F'] = list(model[ngram].fa.values())
                                df_details['fit=a*w^b'] = model[ngram].temp_fa
                except Exception as e:
                    print(f"Error getting details ngram: {e}")
                    df_details = None 

        # --- Save Details DataFrame to Buffer (if exists) --- 
        details_buffer = None
        if details_filename and df_details is not None and not df_details.empty:
            details_buffer = io.BytesIO()
            with pd.ExcelWriter(details_buffer, engine='xlsxwriter') as writer_details:
                df_details.to_excel(writer_details, index=False, sheet_name='Details')
            details_buffer.seek(0)
            
        # --- Prepare Download Data (Single file or ZIP) --- 
        download_data = None
        download_filename = main_filename
        status_message = [f"Main file ready: {main_filename}"]
        
        if details_buffer:
            # Create ZIP archive if details file exists
            zip_buffer = io.BytesIO()
            with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zipf:
                zipf.writestr(main_filename, main_buffer.getvalue())
                zipf.writestr(details_filename, details_buffer.getvalue())
            zip_buffer.seek(0)
            
            download_filename = "{}_results.zip".format(os.path.splitext(file)[0])
            download_data = dcc.send_bytes(zip_buffer.getvalue(), filename=download_filename)
            status_message = [f"ZIP archive ready: {download_filename} (contains main results and details)"]
        else:
            # Send only the main file if no details
            download_data = dcc.send_bytes(main_buffer.getvalue(), filename=main_filename)
            
        return html.Div(status_message, style={'color': 'green'}), download_data

    except Exception as e:
        print(f"Error during save: {str(e)}")
        import traceback
        traceback.print_exc() # Print detailed traceback
        return html.Div([f"Error saving data: {str(e)}"], style={'color': 'red'}), dash.no_update
